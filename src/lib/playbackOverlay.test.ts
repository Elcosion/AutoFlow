...
const status = (patch: Partial<MacroPlaybackStatus> = {}): MacroPlaybackStatus => ({
  running: true,
  currentStep: 2,
  totalSteps: 5,
  playbackId: 7,
...
describe("playback overlay status mapping", () => {
  it("only shows for an active or recent playback snapshot", () => {
    ...
    expect(
      shouldShowPlaybackOverlay(
        status({ running: false, phase: "cleanup_failed", overlayVisible: true }),
      ),
    ).toBe(true);
  });
  ...
  it("uses high-frequency polling only while the overlay is visible", () => {
    ...
    expect(
      playbackPollInterval(status({ overlayVisible: false })),
